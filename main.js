...
function move(blockid1,blockid2){
	...
	if(isCheckMate()){
		alert("checkmate !!!")
		document.getElementById("successMsg").innerHTML="game finished!!";
		gameFinished();
		//finished()
	}
		...
		if(isCheck("w",white[4].pos)!==undefined){
			...
			white.forEach((ele)=>{
				...
				while(moves.length!=0){	
					ans.push(virtualmove(ele.pos,moves.pop(),"w"))
				}
			})

			// TO CHECK FOR ALL PLAYERS VIRTUAL MOVES LEAD TO CHECK OR NOT
			var val=true;
			ans.forEach((ele)=>{
				if(ele==true)
					val=false;
		...
		if(isCheck("b",black[3].pos)!==undefined){
			...
			black.forEach((ele)=>{
				...
				while(moves.length!=0){
					...
					ans.push(virtualmove(ele.pos,moves.pop(),"b"))
				}
			})
